...
model_savefile = "models/q_cnn_color.pth"
save_model = True
load_model = True
skip_learning = True
...
def create_simple_game():
    ...
    game.set_screen_format(vzd.ScreenFormat.RGB24)  # RGB color
...
def run(game, agent, actions, num_epochs, frame_repeat, steps_per_epoch=2000):
    ...
    for epoch in range(num_epochs):
        ...
        for _ in trange(steps_per_epoch, leave=False):
            ...
            if not done:
                next_state = preprocess_rgb(game.get_state().screen_buffer, resolution)
            else:
                next_state = np.zeros((3, 96, 128)).astype(np.float32)
...
class DuelQNet(nn.Module):
    ...
    def __init__(self, available_actions_count):
        ...
        self.state_fc = nn.Sequential(nn.Linear(4104, 64), nn.ReLU(), nn.Linear(64, 1))

        self.advantage_fc = nn.Sequential(
            nn.Linear(4104, 64), nn.ReLU(), nn.Linear(64, available_actions_count)
        )
    ...
    def forward(self, x):
        ...
        x = self.conv4(x)
        x = x.view(-1, 8208)
        x1 = x[:, :4104]  # Input for the net to calculate the state value
        x2 = x[:, 4104:]  # Relative advantage of actions in the state
