...
RGB input | CNN backbone + MLP Actor/Critic heads.
"""
...
def create_simple_game():
    ...
    game.set_screen_format(vzd.ScreenFormat.RGB24)         # 3-channel RGB
...
class ActorCriticNet(nn.Module):
    ...
    Input:  (B, 3, 30, 45)  RGB frame
    CNN:    Conv(3→32) → Conv(32→64) → Conv(64→64)  (stride-2, padding-1 each)
    ...
    def __init__(self, action_size: int):
        ...
        # Shared CNN backbone — 3 input channels for RGB
        self.cnn = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1),   # → (B,32,15,23)
            nn.ReLU(),
...
def test(game, agent, actions, num_episodes: int = 10):
    print("\nTesting...")
    scores = []
